...
class BoundingBox(BaseModel):
    ...
    def split_into_squares(self, size_in_degrees: Decimal) -> List["BoundingBox"]:
        ...
        dim = math.ceil(bbox.height / subcell_length)

        # Subdivide bounding box into squares
        squares = []
        for i in range(dim):
            slice_min_x = float(self.min_x + (i * subcell_length))
            slice_max_x = slice_min_x + float(subcell_length)
            for j in range(dim):
                slice_min_y = float(self.min_y + (j * subcell_length))
                slice_max_y = slice_min_y + float(subcell_length)
                squares.append(
                    BoundingBox(
                        min_x=round(Decimal(slice_min_x, context), 6),
                        max_x=round(Decimal(slice_max_x, context), 6),
...
def convert_degrees_to_meters(
    meters: float, coord: WGS84Coordinate
) -> Tuple[float, float]:
    """Converts a distance expressed in degrees into
    units of meters at a point on the WGS 84 datum.
    ...
    Returns:
        ((`float`, `float`)): A two-item tuple consisting of the
            meters latitude and longitude of the converted
            distance, respectively.
    """
    # Convert distance for latitude
    meters_lat = meters * 111000

    # Convert distance for longitude
    latitude_radians = math.radians(float(coord.lat))
    meters_lon = meters * 111000 * math.cos(latitude_radians)
