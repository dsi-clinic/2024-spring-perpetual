...
def create_df(file_name, city, state):
    """
    Create a business dataframe based on business sales data from a particular year
    Args:
...
def find_top_businesses(business_df, foot_df, year, radius_km=1.0):
    """
    Find the top businesses in the city based on foot traffic surrounding it
    Args:
        business_df - business dataframe (DataFrame)
        foot_df - foot traffic dataframe (DataFrame)
        year - the year (int)
...
def find_top_unique_business_addresses(business_df, foot_df, year, min_unique_businesses=100, max_radius_km=1.0, radius_step=0.1):
    ...
    Args:
        ...
        foot_df - foot traffic dataframe (DataFrame)
        year - the year (int)
        min_unique_businesses - minimum unique business locations to output (int)
...
def morph_and_visualize_business(df):
    ...
    joint_axes = sns.jointplot(x='Longitude', y='Latitude', hue='Total Foot Traffic', data=df_subset, s=0.5)
