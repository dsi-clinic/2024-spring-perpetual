...
def x_highest_visits(df, x):
    """
    Takes a city foot traffic dataframe and an x (int) and outputs the top x 
    locations by raw visit counts
    """
...
def morph_and_visualize(df):
    """
    Takes a city foot traffic dataframe, estructure the dataset so each row 
    represents a place visit, and visualize foot traffic
    """
